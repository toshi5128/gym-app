...
describe('ネスプレッソ ヴァーチュオ', () => {
  ...
  it('氷入りで最終的にグラス1杯＝500ml として数える', () => {
    // 水分は「口に入る量」で数える。氷は溶ければ水になるので含める
    expect(v.ml).toBe(500)
  })

  it('カフェインはカプセル由来なので氷を足しても増えない（170mg）', () => {
    expect(v.caffeineMg).toBe(170)
  })

  it('ヴァーチュオ1杯だけで、1日の水分の1本ぶん（500ml）が埋まる', () => {
    const c = K.waterCups(v.ml, 3900, 500)
    expect(c.done).toBe(1)
  })
