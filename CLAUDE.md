...
## デザイン指針（厳守）
- **エメラルド版（2026-07-08〜）**：金×緑×黒×白。背景=深いエメラルド`--bg:#0e2c22`（glow`#164034`）/ カード`--surface:#16402f` / 文字=白`--txt:#f2f7f3`（副`#c2d1c8`/薄`#8ba99b`）/ 主役=ゴールド`--blue:#cba24e`。トークンは `:root[data-theme="light"]` に定義（命名は名残・applyThemeが常にlight固定）。
- **緑は「伸び・達成」の差し色**：`--pos:#57cf94`＝先週比プラス(`.wk-sub.up`)・PR更新数。金＝ロゴ/ボタン/アクティブタブ/自己ベスト演出(`.prflash`＝金の箱)。
- ライムイエロー禁止（Burnfit的なのが嫌い）。深いエメラルドのみ。クリーン＆余白（Apple Fitness系）の高級感。
- 絵文字は使わない（ダサい＝NG）。アイコンは線SVGで統一。
- 旧：アイボリー`#f1f0ec`ライト固定だったが、下田さんの好み（金黒白緑）でエメラルド地×白文字へ全面変更。
